...
import com.example.demorest.domain.EventStatus;
...
class EventControllerTest extends BaseControllerTest {
	...
	String getBearerToken() throws Exception {
		Account account = Account.builder()
				.email(appProperites.getAdminName())
				.password(appProperites.getAdminPassword())
				.roles(Set.of(AccountRole.ADMIN, AccountRole.USER))
				.build();
		accountService.saveAccount(account);
		
		var body = mockMvc.perform(post("/oauth/token")
	...
	void postEvent() throws Exception {
		EventDto event = EventDto.builder()
				...
				.endEventDateTime(LocalDateTime.of(2020, 6, 5, 18, 0))
				.basePrice(100)
				.maxPrice(200)
		...
		mockMvc.perform(post("/api/events")
			...
			.andDo(document("create-event",
				...
				requestFields(
					...
					fieldWithPath("beginEnrollmentDateTime").description("Begin date time of the enrollment"),
					fieldWithPath("closeEnrollmentDateTime").description("Close date time of the enrollment"),
					...
					fieldWithPath("limitOfEnrollment").description("The number of people for the event")),
				responseHeaders(
	...
	void postEvent_Bad_Request_Invalid_Params() throws Exception {
		EventDto event = EventDto.builder()
				...
				.beginEventDateTime(LocalDateTime.of(2020, 6, 1, 8, 0))
				.endEventDateTime(LocalDateTime.of(2020, 5, 5, 18, 0))
				.basePrice(1000)
				.maxPrice(200)
				.limitOfEnrollment(200)
				.location("Seoul")
	...
	public void putEvent() throws Exception {
		...
		var body = this.mockMvc.perform(post("/api/events")
		...
		this.mockMvc.perform(put("/api/events/{id}", eventId)
				.header(HttpHeaders.AUTHORIZATION, token)
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaTypes.HAL_JSON)
